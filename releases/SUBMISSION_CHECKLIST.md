# MCP Host Submission Checklist

Use this checklist before submitting a reviewed MCP Host artifact to an MCP Host
marketplace.
...
- Local standalone validation passed:
  - `mcp_host_skills_tests.py: 7 tests passed`
  - `package_validation_tests.py: 5 tests passed`
- OAuth sign-in, MCP Host client smoke tests, and write-flow validation are
  marked complete.
...
- [x] Confirm the MCP Host artifact path is correct: `codex/` for Codex or `claude/` for Claude.
- [x] Confirm the MCP Host manifest is present and valid JSON.
...
- [x] Run the MCP Host-specific validation flow.
- [x] Complete OAuth authentication in the MCP Host client.
...
- [x] Confirm the visible MCP Host tool list matches the tool-to-scope mapping in `../shared/OAUTH_SCOPES.md`.
...
- [x] Confirm known limitations are documented in MCP Host-appropriate copy.
...
- [ ] Confirm CODEOWNERS covers MCP Host Skill and configuration paths with the required security review owners.
- [ ] Confirm the staging-to-public copy pipeline runs validation tests as a required gate.

## Release Record

- [x] Create an MCP Host-specific release manifest.
- [x] Record the MCP Host name.
- [x] Record the submitted artifact path.
- [x] Record the MCP Host-scoped version tag.
